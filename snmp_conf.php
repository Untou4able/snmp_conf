...
class snmp_conf {
    ...
    protected function make_req() {
        $this->snmp_req = '/usr/local/bin/'. $this->action.' -t '.$this->timeout.' -r '.$this->retries.' -v '. $this->version.' -c '. $this->snmp_comm.' '. $this->output.' '. $this->ip.' '.$this->object_id.' '.$this->type.' '.$this->value.' '. $this->redirect.' '. $this->addition;
    }
